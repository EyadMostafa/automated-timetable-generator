...
from schemas import (
    ...
    SessionType, Course, Section, Major,
...
class CSPSolver:
    ...
    def __init__(self, timetable_data: TimetableData, mode: SolverMode):
        ...
        self.curriculum_map: Dict[Tuple[int, Major], List[str]] = {}
        self.sections_by_major_map: Dict[Tuple[int, Major], List[Section]] = {}
    ...
    def _initialize_internal_lookups(self):
        """
        Initializes the fast internal lookup maps, now fully aware of majors.
        """
        ...
        for curr in self.data.curriculum:
            key = (curr.year, curr.major)
            self.curriculum_map.setdefault(key, []).append(curr.course_id)

        for section in self.data.sections:
            key = (section.year, section.major)
            self.sections_by_major_map.setdefault(key, []).append(section)

        for (year, major), course_ids in self.curriculum_map.items():
            sections_to_schedule = frozenset()
            if major == Major.GENERAL:
                all_sections_in_year = []
                for (sec_year, _), sections in self.sections_by_major_map.items():
                    if sec_year == year:
                        all_sections_in_year.extend(sections)
                sections_to_schedule = frozenset(all_sections_in_year)
            else:
                sections_for_major = self.sections_by_major_map.get((year, major), [])
                sections_to_schedule = frozenset(sections_for_major)

            if not sections_to_schedule: continue
    ...
    def _generate_valid_domains(self, course: Course) -> List[Domain]:
        ...
        if not valid_rooms or not valid_instructors: return []
    ...
    def _backtrack(self, assignment: Assignment, unscheduled_sections_map: Dict[Course, FrozenSet[Section]], start_time: float, timeout_seconds: int) -> Optional[Assignment]:
        # --- DEBUGGING PROBE ---
        remaining_count = sum(len(s) for s in unscheduled_sections_map.values())
        print(f"Entering backtrack... {remaining_count} section-classes remaining.")
        # --- END PROBE ---
        ...
        for domain in domains:
            ...
            for section_group in self._form_valid_groups(sections_to_schedule, domain[DOM_ROOM]):
                ...
                is_valid, _ = is_consistent(variable, domain, assignment)
                if is_valid:
...
if __name__ == '__main__':
    try:
        timetable_data = load_timetable_data_from_excel('/home/eyad/timetable-generator/timetable-generator/Tables - Copy.xlsx')
